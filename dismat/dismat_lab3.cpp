...
long long hash_kod(int &n, int &k) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (preslikavanje.count(boje[i]) == 0)
      preslikavanje[boje[i]] = slj++;
...
int main(void) {
  ...
  set<int> neispravni; // vrhovi koji imaju istu boju kao barem jedan njihov
                       // susjedni vrh
  ...
  while (!izmjene.empty() && !obojiv) {
    ...
    cout << "je li ovakvo stanje već obiđeno prije: "
         << stanja.count(kod_stanja) << '\n';
    cout << "broj jedinstvenih obiđenih stanja: " << stanja.size() << '\n';
#endif

    if (stanja.count(kod_stanja) > 0)
      continue;
